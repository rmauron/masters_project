...
```{r}
#save plot
plotPDF(p1, p2, p3, p4, p5, name = "Harmony-Combined-sample-clusters", addDOC = FALSE)
```
...
pv <- plotMarkers(
    ...
    cutOff = "FDR <= 0.1 & abs(Log2FC) >= 0.5",#"FDR <= 0.1 & abs(Log2FC) >= 0.5",
    plotAs = "Volcano")
#pv
```
...
```{r}
# for the peaks that are more accessible in the "CG" cells by using peaks with Log2FC <= -0.5
...
```{r}
#save plots
plotPDF(ggUp, ggUp_cond, ggDo, name = "Motifs-Enriched-plot", width = 5, height = 5, ArchRProj = projMulti4, addDOC = FALSE)
```
