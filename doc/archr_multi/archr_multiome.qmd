...
nCells_df[order(nCells_df$sample),]
```
...
```{r}
nCells_conditions <- data.frame(sample = projMulti1@cellColData@listData[["Conditions"]]@values,
                        nCells = projMulti1@cellColData@listData[["Conditions"]]@lengths)

nCells_conditions[order(nCells_conditions$sample),] #order the df before printing it
```

```{r}
ggplot(nCells_conditions,
       aes(x = sample, y = nCells)) +
    geom_bar(stat = "identity") +
...
heatmap_gex <- plotMarkerHeatmap(
  ...
  transpose = T,
  labelMarkers = c("Gba", "Lrrk2", "Pink1", "Uchl1", "Vps35")#specify the markers you want a label on
  )
