...
diffmotif <- getMarkerFeatures(
    ...
    useSeqnames = "z",
    groupBy = "CellType_Condition",
    useGroups = "Telenchephalon_interneurons_x_CF",
    bgdGroups = "Telenchephalon_interneurons_x_CG"
)
```

```{r}
li <- as.data.frame(diffmotif@elementMetadata@listData[["name"]])
li$MeanDiff <- as.numeric(abs(diffmotif@assays@data@listData[["MeanDiff"]][,1]))
colnames(li) <- c("name", "MeanDiff")
li <- li[order(-li$MeanDiff),]
