...
# gsub(" ", "-",paste(plot_type, cell_type))

i <- 1
for (cluster in 1:length(markerList@listData)) {
    if (nrow(markerList@listData[[cluster]]["name"]) >= 20) {
        if (i == 1) {
            my_row <- head(markerList@listData[[cluster]]["name"],20)
            df_top_20 <- as.data.frame(my_row)
            colnames(df_top_20)[1] <- gsub(" ", "",paste("C", cluster))
        } else {
        df_top_20 <- cbind(df_top_20, head(markerList@listData[[cluster]]["name"],20))
        colnames(df_top_20)[i] <- gsub(" ", "",paste("C", cluster))
        }
    } else {
        next
    }
