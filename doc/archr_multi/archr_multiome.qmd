...
projMulti4 <- addGroupCoverages(
    ...
    groupBy = "Clusters_ATAC" #in CellColData, 
    )
...
for (i in 1:length(useGroups_list)) {
    ...
    #name <- gsub(" ", "", paste0("marker_pairwise", "_", useGroups_list[i]))
    ...
    nam_significant <- nam_significant[((nam_significant$FDR <= 0.1) & (nam_significant$Log2FC >= 1)),] #& (nam_significant$Log2FC >= 1 | nam_significant$Log2FC <= 1 )),]
...
```{r}
#rename each dataframe in markerTest and in marker_significant
#names are found in "colnames"
for (i in seq_along(markerTest)) {
  names(markerTest[[i]]) <- useGroups_list[i]
  names(marker_significant[[i]]) <- useGroups_list[i]
}
```
...
# #rename each dataframe in motifsUp and motifsDo
# #names are found in "colnames"
# for (i in seq_along(motifsUp)) {
#   names(motifsUp[[i]]) <- useGroups_list[i]
#   names(motifsDo[[i]]) <- useGroups_list[i]
# }
```
...
p <- plotBrowserTrack(
    ArchRProj = projMulti4, 
    groupBy = "predictedGroup_Un_3", 
    geneSymbol = motifs, #specify motifs to plot here
    upstream = 50000,
    downstream = 50000,
