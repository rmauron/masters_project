...
```{r}
#save plots
plotPDF(pv, name = "Conditions-Markers-Volcano", width = 5, height = 5, ArchRProj = projMulti4, addDOC = FALSE)
```
...
```{r}
#To prepare this data for plotting with ggplot we can create a simplified data.frame object containing the motif names, the corrected p-values, and the significance rank
df_cond <- data.frame(TF = rownames(motifsUpCondition), mlog10Padj = assay(motifsUpCondition)[,1])
df_cond <- df_cond[order(df_cond$mlog10Padj, decreasing = TRUE),]
df_cond$rank <- seq_len(nrow(df_cond))
```
...
ggUp <- ggplot(df, aes(rank, mlog10Padj, color = mlog10Padj)) + 
  ...
  scale_color_gradientn(colors = paletteContinuous(set = "comet")) +
    ggtitle("Motif enrichment among every cells")
...
ggUp_cond <- ggplot(df_cond, aes(rank, mlog10Padj, color = mlog10Padj)) + 
  geom_point(size = 1) +
  ggrepel::geom_label_repel(
        data = df_cond[rev(seq_len(30)), ], aes(x = rank, y = mlog10Padj, label = TF), 
        size = 1.5,
        nudge_x = 2,
        color = "black",
  ...
  xlab("Rank Sorted TFs Enriched") +
  scale_color_gradientn(colors = paletteContinuous(set = "comet")) +
    ggtitle("Motif enrichment in conditions")
