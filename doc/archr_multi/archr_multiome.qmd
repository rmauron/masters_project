...
snATAC-seq and snRNA-seq from the same cells/nuclei are available.
...
create an `ArchRProject` object from the ArrowFiles
...
Check how many cells in our project (the ones that have passed scATAC-seq quality control) are not also in our scRNA-seq data
...
And then create clusters for each.
...
Can embed the UMAP of choice with the matrix of choice. 
The result is 1 embedding per marker passed
...
Magic helps having a better signal
Return the same plot but hopefully with better embedding
...
Since we don't have our cells labeled, we need to infer their origin from the clusters we have and the http://mousebrain.org database from Linnarsson lab:
...
### Annotate every cell with a taxonomy rank.
In this project we wanted to see the different taxonomy annotation so we ran the same function 4 times.
The code in fact overwrite this annotation each time so be carefull to add the taxonomy annotation you want to continue with last.
...
### Rename clusters
Clusters can be renamed mannually like below. It has not been done in this study.
```{r}
...
Add an annotation level joining cells by condition and by cluster
...
#### Clusters metrics
Create table with numerical description of the dataset
...
### Groups for differential testing
...
Here, markers are calculated by cluster and condition (CellType_Condition).
...
## Motifs from chromVAR from only the 4 celltypes of interest
