...
```{r}
#plot Volcano of difference between the 2 conditions

# pv <- plotMarkers(seMarker = markerTest[[1]], 
#                  name = useGroups_list[i], 
#                  cutOff = "FDR <= 0.5 & abs(Log2FC) >= 1",
#                  plotAs = "Volcano")
# print(pv) #print the plot
...
#Volcano plot
cell_types <- sort(unique(projMulti4@cellColData@listData[["predictedGroup_Un_3"]]))

for (marker in seq_along(markerTest)) {
pv <- plotMarkers(
    seMarker = marker, #iterate in markerTest
    name = colnames, #change cell name
    cutOff = "FDR <= 0.1 & Log2FC >= 1",
    plotAs = "Volcano" #MA
    ) + ggtitle(cell_types[marker])
